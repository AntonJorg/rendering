...
fn sample_point_light(pos: vec3f, seed: ptr<function, u32>) -> Light {

    var idx: u32;
    if (rnd(seed) < 0.5) {
        idx = u32(uniforms.lightIdx1);
    } else {
        idx = u32(uniforms.lightIdx2);
    }

    let face = meshFaces[idx];
    ...
    let beta = (1.0 - r2) * sqrt_r1;
    let gamma = r2 * sqrt_r1;

    let sampled_pos = alpha * v0 + beta * v1 + gamma * v2;
    let sampled_normal = normalize(alpha * n0 + beta * n1 + gamma * n2);

    let material = materials[face[3]];

    let L_e = material.emission.rgb;

    let dir = sampled_pos - pos;
    let dist = length(dir);

    let L_i = max(dot(sampled_normal, -normalize(dir)), 0.0) * L_e * area * 2 / (dist * dist);
...
fn lambertian(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
    ...
    let light = sample_point_light(hit.position, seed);
    
    var L_e: vec3f;
    if (hit.emit) {
        L_e = hit.color.ambient;
    } else {
        L_e = vec3f(0.0);
    }

    var L_r = ((hit.color.diffuse * 0.9) / 3.14159) * light.L_i * dot(hit.normal,light.w_i);
    ...
    hit.rgb_factor *= hit.color.diffuse;

    let P_d = (hit.rgb_factor.r + hit.rgb_factor.g + hit.rgb_factor.b) / 3.0;
    ...
    if (rnd(seed) < P_d) {
        ...
        let phi = 2 * 3.14159 * r2;

        let direction = spherical_direction(sin(theta), cos(theta), phi);
        let rotated_direction = rotate_to_normal(hit.normal, direction);

        r.direction = rotated_direction;
        hit.has_hit = false;
        hit.emit = false;
...
fn phong(ray: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
    let light = sample_point_light(hit.position, seed);
...
fn refraction(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
...
fn glossy(ray: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
    
    let r = refraction(ray, hit, seed);
    let p = phong(ray, hit, seed);
...
fn shade(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
    switch hit.shader {
        ...
        case 2 { return phong(r, hit, seed); }
        case 3 { return mirror(r, hit, seed); }
        case 4 { return refraction(r, hit, seed); }
        case 5 { return glossy(r, hit, seed); }
...
fn main_fs(@builtin(position) fragcoord: vec4f, @location(0) coords: vec2f) -> FSOut {
    ...
    let prog_jitter = vec2f(rnd(&t), rnd(&t))/(f32(height));
    ...
    var hit = HitInfo(false, 0.0, vec3f(0.0), vec3f(0.0), color, 1, 1.0, true, vec3f(1.0));
    for(var i = 0; i < max_depth; i++) {
        if(intersect_scene(&r, &hit)) { 
            result += hit.rgb_factor * shade(&r, &hit, &t);

        } else {
            result += hit.rgb_factor * uniforms.background_color.rgb;
            break;
        }
    ...
    var fsOut: FSOut;
    fsOut.frame = vec4f(pow(accum_color, vec3f(1.0/uniforms.gamma)), uniforms.background_color.a);

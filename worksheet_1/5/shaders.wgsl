...
fn lambertian(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> vec3f {
    let light_pos = uniforms.scene.light.position;
    let light_int = uniforms.scene.light.intensity;
    
    let l_i = light_int / length(light_pos - (*hit).position);

    let p_d = 2.0;
    
    let angle_of_incidence = dot((*hit).normal, (*r).direction);

    let l_o = p_d / 3.1415 * l_i * angle_of_incidence;

    return (*hit).color * l_o;
}

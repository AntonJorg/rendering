...
fn intersect_triangle(ray: Ray, hit: ptr<function, HitInfo>, triangle_idx: u32) -> bool {
    ...
    let vertices = array<vec3f, 3>(
        vertexPositions[vertex_idxs[0]],
        vertexPositions[vertex_idxs[1]],
        vertexPositions[vertex_idxs[2]],
    );
    ...
    let o_to_v0 = vertices[0] - ray.origin;
    ...
    if ((ray.tmin < t) & (t < ray.tmax)) {
        hit.dist = t;
        hit.has_hit = true;
        hit.color = Color(vec3f(0.0), vec3f(0.0), vec3f(0.0));
